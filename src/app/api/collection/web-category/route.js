...
export async function GET(request) {
    ...
    try {
        ...
        if (category === "men") {
            const collections = await MenModel1.find({status:"active"});
            ...
            const collections = await WomenModel1.find({status:"active"});
            collectionsWithProductCount = await Promise.all(
                collections.map(async (collection) => {
                    const productCount = await ProductModel1.countDocuments({ collectionName: collection.title });
                    return { ...collection.toObject(), productCount }
            ...
            const collections = await KidModel1.find({status:"active"});
            collectionsWithProductCount = await Promise.all(
                collections.map(async (collection) => {
                    const productCount = await ProductModel1.countDocuments({ collectionName: collection.title });
                    return { ...collection.toObject(), productCount }
            ...
            const collections = await AccessoriesModel1.find({status:"active"});
            collectionsWithProductCount = await Promise.all(
                collections.map(async (collection) => {
                    const productCount = await ProductModel1.countDocuments({ collectionName: collection.title });
                    return { ...collection.toObject(), productCount }
