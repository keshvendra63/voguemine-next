...
async function getAllUrls() {
  ...
  const men = await Men.find({}).select('handle -_id');
  const women = await Women.find({}).select('handle -_id');
  const kids = await Kid.find({}).select('handle -_id');
  const accessories = await Accessories.find({}).select('handle -_id');
