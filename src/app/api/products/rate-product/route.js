...
export async function PUT(req){
  ...
  await connectDb2()
        ...
        const rateProduct = await ProductModel1.findByIdAndUpdate(
            prodId,
            {
                $push: {
                    ratings: {
        ...
        const rateProduct1 = await ProductModel2.findByIdAndUpdate(
            prodId,
            {
                $push: {
                    ratings: {
        ...
        const product = await ProductModel1.findById(prodId);
        ...
        const product1 = await ProductModel2.findById(prodId);
        ...
        const updatedProduct = await ProductModel1.findByIdAndUpdate(
            prodId,
            { totalrating: averageRating },
            { new: true }
        );
        const updatedProduct1 = await ProductModel2.findByIdAndUpdate(
            prodId,
