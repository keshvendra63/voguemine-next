...
export async function GET(request) {
  ...
  try {
    ...
    const initialResults = await ProductModel1.find(baseQuery);
    ...
    const filteredProducts = await ProductModel1.find(filteredQuery)
      .sort(sortCriteria)
      .skip((page - 1) * limit)
      .limit(limit);

    const totalProducts = await ProductModel1.countDocuments(filteredQuery);
