  ...
  try {
    ...
    if (order && order2){
      await sendVariableMessage({
        ...
        templateId: "01jy8n97yr44pdd1mncfts1zhp",
        parameterValues: {
                "0": order1?.orderNumber,
                "1": `${order1?.finalAmount}`,
                "2": `${order1?.orderItems?.length}`,
                "3": order1?.orderType
            }
      });
      await sendVariableMessage({
      ...
      await sendVariableMessage({
        ...
        journeyId: process.env.WHATSAPP_JOURNEY_ID,
        templateId: "01jy8h1wvys3t4v1t4215wzzcz",
        parameterValues: {
                "0": shippingInfo?.firstname,
                "1": order1?.orderNumber,
                "2": `${order1?.finalAmount}`,
                "3": orderItemsString
        },
        buttons: [
              {
    ...
    // Schedule a message after 3 hours
     setTimeout(async () => {
       ...
       await sendVariableMessage({
        ...
        to: `91${shippingInfo.phone}`,
        journeyId: process.env.WHATSAPP_JOURNEY_ID,
        templateId: "01jy8h8z0t52bregyk55tkt3k2",
        parameterValues: {
                "0": shippingInfo.firstname,
                "1": order1?.orderNumber,
            }
