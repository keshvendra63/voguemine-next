...
const startOfDayIST = new Date(today);
  ...
  await connectDb()
    const monthdata=await OrderModel1.aggregate([
      {
        $match:{
          createdAt:{
            $lte:new Date(),
    ...
    const yeardata=await OrderModel1.aggregate([
        {
          $match:{
      ...
      const todaydata = await OrderModel1.aggregate([
        {
          $match: {
            createdAt: {
              $gte: startOfDay,
      ...
      const weekdata = await OrderModel1.aggregate([
        {
          $match: {
            createdAt: {
              $gte: startOfWeek,
      ...
      const yesterdaydata = await OrderModel1.aggregate([
        {
          $match: {
            createdAt: {
              $gte: yesterday,
