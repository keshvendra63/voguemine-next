...
import {LocationModel1} from "../../../../../models/locationModel";
export async function GET() {
  ...
  await connectDb()
  const monthdata = await LocationModel1.aggregate([
    {
      $match: {
        createdAt: {
          $lte: new Date(),
  ...
  const yeardata = await LocationModel1.aggregate([
    {
      $match: {
  ...
  const todaydata = await LocationModel1.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfDay,
  ...
  const weekdata = await LocationModel1.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfWeek,
  ...
  const yesterdaydata = await LocationModel1.aggregate([
    {
      $match: {
        createdAt: {
          $gte: yesterday,
