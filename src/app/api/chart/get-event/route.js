...
import {EventsModel1} from "../../../../../models/eventsModel";
export async function GET() {
  ...
  await connectDb()
  const monthdata = await EventsModel1.aggregate([
    {
      $match: {
        createdAt: {
          $lte: new Date(),
  ...
  const yeardata = await EventsModel1.aggregate([
    {
      $match: {
  ...
  const todaydata = await EventsModel1.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfDay,
  ...
  const weekdata = await EventsModel1.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfWeek,
  ...
  const yesterdaydata = await EventsModel1.aggregate([
    {
      $match: {
        createdAt: {
          $gte: yesterday,
