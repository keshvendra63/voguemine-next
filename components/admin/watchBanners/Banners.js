...
setUser(JSON.parse(localStorage.getItem("user")))
    ...
    // const queryParams = new URLSearchParams(location.search);
      useEffect(()=>{
        ...
        searchParams.set('pageName',"watchbanners");
    ...
    const fetchBanners = async () => {
      try {
        const response = await fetch(
          `/api/watchBanner/get-banners?id=67f4b05ef8a4115e4cca5a13`
        );
...
try{
const response=await fetch(`/api/watchBanner/update-banners?id=67f4b05ef8a4115e4cca5a13&token=${user?.token}`,{
    method:"PUT",
