...
try{
const response=await fetch(`/api/collection/get-single-collection?id=${getCollectionId}&category=${category}`)
...
if(response.ok){
  ...
  setImg2(data?.images[data?.images?.length - 1])
  ...
  const fetchProducts=async()=>{
    try{
    const response=await fetch(`/api/products?collectionHandle=${data?.handle}&limit=10000`)
...
const handleCollectionSave=async()=>{
  ...
  if(getCollectionId===""){
    try{
      const response = await fetch(`/api/collection/create-collection?category=${category}&token=${user?.token}`, {
        method:"POST",
        headers: { "Content-Type": "application/json" },
        body:JSON.stringify({title,category:category1,handle,isTrending,images:[img1,img2],metaDesc,metaTitle,mostTrending,status,order})
    }
      )
      if(response.ok){
        toast.success(`Collection Created Successfully Under ${category}`)
        createHistory({ name: user?.firstname, title: title, sku: category, productchange: `Created a Collection`, time: new Date() })
        setPrdtOpens(false)
    ...
    try{
      const response = await fetch(`/api/collection/update-collection?category=${category}&id=${getCollectionId}&token=${user?.token}`, {
        method:"PUT",
        headers: { "Content-Type": "application/json" },
        body:JSON.stringify({title,category:category1,handle,isTrending,images:[img1,img2],metaDesc,metaTitle,mostTrending,status,order})
    }
      ...
      if(response.ok){
        toast.success(`Collection Updated Successfully Under ${category}`)
        createHistory({ name: user?.firstname, title: title, sku: category, productchange: `Updated a Collection`, time: new Date() })
      }
      else{
...
const handleCollectionDelete=async()=>{
  if(getCollectionId!=="" && category!==""){

  
  try{
    const response = await fetch(`/api/collection/delete-collection?category=${category}&id=${getCollectionId}&token=${user?.token}`, {
      method:"DELETE",
      headers: { "Content-Type": "application/json" },
  }
    )
    if(response.ok){
      toast.success(`Collection Deleted Successfully Under ${category}`)
    createHistory({ name: user?.firstname, title: title, sku: category, productchange: `Deleted a Collection`, time: new Date() })
    ...
    <div className={styles.singleCol}>
      ...
      <div className={styles.singleColInfo}>
        ...
        <div style={{display:getCollectionId===""?"block":"none"}}>
            ...
            <div>
              <p>Is Trending</p>
              <select name="" id="" value={isTrending} onChange={(e)=>setIsTrending(e.target.value)}>
