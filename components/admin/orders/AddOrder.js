    ...
    if (window.confirm("Do you want to mark this order as Delivered?")) {
      try {
        const response = await fetch(`/api/order/send-delivery`,{
          method:"PUT",
          headers: { "Content-Type": "application/json" },
          body:JSON.stringify({name:`${orderState?.shippingInfo?.firstname}`,ordernumber:`${orderState?.orderNumber}`,email:`${orderState?.shippingInfo?.email}`,orderId:orderState?._id, phone:orderState?.shippingInfo?.phone,arriving:false})
        })
        if(response.ok){
