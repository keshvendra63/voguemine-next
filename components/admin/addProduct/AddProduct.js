...
if(formik.values.title==="" || formik.values.handle==="" || formik.values.description==="" || formik.values.price==="" || formik.values.category==="" || formik.values.brand==="" || formik.values.state==="" || formik.values.sku==="" || formik.values.collectionName===""){
  ...
  const handleVariantChange = (index, field, value) => {
    if(getProductId!==""){
      if(user?.firstname==="parul" || user?.firstname==="Ankit Singh" || user?.firstname==="ujjawal"){
        setVariants((prevVariants) => {
          const updatedVariants = [...prevVariants];
...
>
  ...
  name="collectionName"
  onChange={(e) => {
    const selectedCollection = collectionState.find(
      (collection) => collection.title === e.target.value
    );
    if (selectedCollection) {
      // Update the product collection and handle based on the selected collection
      setproductCollectionName(selectedCollection.title);
      setCollectionHandle(selectedCollection.handle);
    }
  }}
        ...
        {collection.title}
            ...
            <div className={styles.title}>
                ...
                <input type="text"
                  placeholder="Enter Meta Title"
                  name="metaTitle1"
                  onChange={(e)=>setmetaTitle1(e.target.value)}
                  value={formik.values.metaTitle1} />
              </div>
              <div className={styles.title}>
                <p>Meta Description</p>
                <textarea type="text"
                  placeholder="Enter Meta Description"
                  name="metaDesc1"
                  onChange={(e)=>setmetaDesc1(e.target.value)}
                  value={formik.values.metaDesc1} />
              </div>
             
            </div>
            <div className={styles.metaDetails}>
              <p>Chicoline Information</p>
            <div className={styles.title}>
                <p>Meta Title</p>
                <input type="text"
                  placeholder="Enter Meta Title"
                  name="metaTitle4"
                  onChange={(e)=>setmetaTitle4(e.target.value)}
                  value={formik.values.metaTitle4} />
              </div>
              <div className={styles.title}>
                <p>Meta Description</p>
                <textarea type="text"
                  placeholder="Enter Meta Description"
                  name="metaDesc4"
                  onChange={(e)=>setmetaDesc4(e.target.value)}
