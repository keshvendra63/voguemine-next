...
const WatchOrders = (props) => {
  ...
  const router = useRouter()
      // const queryParams = new URLSearchParams(location.search);
        useEffect(()=>{
          ...
          searchParams.set('pageName',"watchOrders");
  ...
  useEffect(() => {
    const getOrders = async () => {
      try {
        const response = await fetch(`/api/user/getallorders?page=${page}&isWatchOrders=true`)
  ...
  const fetchSearch = () => {
    const getOrders = async () => {
      try {
        const response = await fetch(`/api/user/getallorders?search=${searchValue}&isWatchOrders="true"`)
        const data = await response.json()
        if (response.ok) {
          setorderState(data.orders)
        }
  ...
  const confirmOrder =async (orderData) => {
    try {
      const response = await fetch(`/api/watchOrder/confirm-order?id=${orderData?._id}&token=${user?.token}`,{
        method:"PUT",
  ...
  const cancelOrder =async (orderData) => {
    if (orderData?.orderType === ("COD") || orderData?.orderType === ("Prepaid")) {
      if (window.confirm("Do you want to cancel?")) {
        try {
          const response = await fetch(`/api/watchOrder/cancel-order?id=${orderData?._id}&token=${user?.token}`,{
            method:"PUT",
            headers: { "Content-Type": "application/json" },
          })
          if (response.ok) {
...
const exportToExcel = () => {
  ...
  const formattedOrders = ordersToExport.map(order => ({
    ...
    'First Name': `${order.shippingInfo.name}`,
              ...
              <p className={`${styles.state} ${item?.orderType === "COD" ? styles.cod : item?.orderType === "Cancelled" ? styles.cancel : styles.paid}`}>{item?.orderType}{item?.isPartial === true ? " / Paid" : ""}</p>
                <div className={styles.productImg} onClick={(e)=>openOrder(item?._id)} style={{cursor:"pointer"}}>
                  ...
                  <div className={styles.detail}>
                    ...
                    <p style={{ display: 'flex', alignItems: 'center' }}>TICKOOCLOCK {
