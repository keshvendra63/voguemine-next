    ...
    if (window.confirm("Do you want to mark this order as Delivered?")) {
      try {
        const response = await fetch(`/api/watchOrder/send-delivery`,{
          method:"PUT",
          headers: { "Content-Type": "application/json" },
          body:JSON.stringify({name:`${orderState?.shippingInfo?.name}`,ordernumber:`${orderState?.orderNumber}`,email:`${orderState?.shippingInfo?.email}`,orderId:orderState?._id})
        })
        if(response.ok){
...
const handleClose1=async()=>{
  ...
  try {
    const response = await fetch(`/api/watchOrder/send-tracking`,{
      method:"PUT",
      headers: { "Content-Type": "application/json" },
      body:JSON.stringify({name:`${orderState?.shippingInfo?.name}`,ordernumber:`${orderState?.orderNumber}`,partner:`${trackpartner}`,link:` TrackingId: ${ tracknumber}, Tracking Link: ${ tracklink} `,email:`${orderState?.shippingInfo?.email}`,orderId:orderState?._id})
    })
    if(response.ok){
