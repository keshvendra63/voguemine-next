...
const createHistory=async(data)=>{
  ...
  const pdfDownload = async () => {
    ...
    for (let i = 0; i < splitOrders.length; i++) {
      ...
      generatePDF(targetRef, { filename: `tickooclock_order_${splitOrder.orderNumber}_${i + 1}.pdf` });
  ...
  const cancelClick = async() => {
    if(orderState?.orderType===("COD")||orderState?.orderType===("Prepaid")){
      if (window.confirm("Do you want to cancel?")) {
        try {
          const response = await fetch(`/api/watchOrder/cancel-order?id=${orderState?._id}&token=${user?.token}`,{
            method:"PUT",
            headers: { "Content-Type": "application/json" },
          })
          if (response.ok) {
      ...
      if (window.confirm("Do you want to Retrieve?")) {
        try {
          const response = await fetch(`/api/watchOrder/retrieve-order?id=${orderState?._id}&token=${user?.token}`,{
            method:"PUT",
            headers: { "Content-Type": "application/json" },
          })
          if (response.ok) {
  ...
  const markReturn = async() => {
    if (window.confirm("Do you want to mark this order as Returned?")) {
      try {
        const response = await fetch(`/api/watchOrder/return-order?id=${orderState?._id}&token=${user?.token}`,{
          method:"PUT",
          headers: { "Content-Type": "application/json" },
        })
        if (response.ok) {
  ...
  const markPaid =async () => {
    if(orderState?.orderType==="COD"){
      if (window.confirm("Do you want to make Prepaid?")) {
        try {
          const response = await fetch(`/api/watchOrder/prepaid-order?id=${orderState?._id}&token=${user?.token}`,{
            method:"PUT",
            headers: { "Content-Type": "application/json" },
          })
          if (response.ok) {
    ...
    if(orderState?.orderType==="Prepaid"){
      if (window.confirm("Do you want to make COD?")) {
        ...
        try {
          const response = await fetch(`/api/watchOrder/cod-order?id=${orderState?._id}&token=${user?.token}`,{
            method:"PUT",
            headers: { "Content-Type": "application/json" },
          })
          if (response.ok) {
  ...
  const markDelivered=async()=>{
    if (window.confirm("Do you want to mark this order as Delivered?")) {
      try {
        const response = await fetch(`/api/watchOrder/send-delivery`,{
          method:"PUT",
          headers: { "Content-Type": "application/json" },
          body:JSON.stringify({name:`${orderState?.shippingInfo?.name}`,ordernumber:`${orderState?.orderNumber}`,email:`${orderState?.shippingInfo?.email}`,orderId:orderState?._id,phone:orderState?.shippingInfo?.phone,arriving:false})
        })
        if(response.ok){
  ...
  const sendArriving=async()=>{
    if (window.confirm("Do you want to mark this order as arriving today?")) {
      try {
  ...
  const markConfirm=async()=>{
    try {
      const response = await fetch(`/api/watchOrder/confirm-order?id=${orderState?._id}&token=${user?.token}`,{
        method:"PUT",
        headers: { "Content-Type": "application/json" },
      })
      if (response.ok) {
  ...
  useEffect(() => {
    if(orderState?.shippingInfo?.email!=="" || orderState?.shippingInfo?.email!==undefined){
      const fetchOrdersByEmail = async () => {
        try {
          const response = await fetch(`/api/watchOrder/get-orders?email=${orderState?.shippingInfo?.email}`, {
            method: "GET",
            headers: {
...
const [msg,setMsg]=useState("")
  const messagePost=async()=>{
    if(msg===""){
      ...
      try {
        const response = await fetch(`/api/watchOrder/set-msg`,{
          method:"PUT",
          headers: { "Content-Type": "application/json" },
...
const handleClose1=async()=>{
  ...
  try {
    const response = await fetch(`/api/watchOrder/send-tracking`,{
      method:"PUT",
      headers: { "Content-Type": "application/json" },
      body:JSON.stringify({name:`${orderState?.shippingInfo?.name}`,ordernumber:`${orderState?.orderNumber}`,partner:`${trackpartner}`,link:` TrackingId: ${ tracknumber}, Tracking Link: ${ tracklink} `,email:`${orderState?.shippingInfo?.email}`,orderId:orderState?._id, phone:orderState?.shippingInfo?.phone, trackingId: tracknumber,trackingLink:tracklink})
    })
    if(response.ok){
...
<p>Total</p>
       ...
       <div className={styles.rightSection}>
        ...
        <p className={styles.customerName}><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTXPgCd93aiNba8lHWJYHQ1C1YwBPwcH_NUmw&usqp=CAU" alt="" /><p>{orderState?.shippingInfo?.name} {orderState?.shippingInfo?.lastname}</p></p>
        ...
        <div className={styles.email}>
          ...
          <p>{orderState?.shippingInfo?.name}</p>
...
<div>
    ...
    <p>{formattedDate}</p>
      ...
      <div className={styles.detail}>
        <div>
        <p>SHIP TO</p>
        <p>{currentOrderState?.shippingInfo?.name}</p>
      ...
      <div className={styles.main}>
        ...
        <p>Thank you shopping with TICKOOCLOCK!</p><br/>
        <p  style={{fontWeight:700}}>TICKOOCLOCK</p>
        <p>H 119, Noida, UP, 201301, India</p>
        <p>contact@tickooclock.com</p>
        <p>+91 9899202079</p>
        <p>tickooclock.com</p>
        </div>
      </div>
